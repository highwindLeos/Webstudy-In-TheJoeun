...
public class CDLib extends CDinfo implements ILendable {
	...
	public void checkOut(String borrower, String checkOutDate) { // 대출
		...
		System.out.println("\"" + super.getCdTitle() +"\" CD 가 대출 되었습니다.");
	...
	public void checkIn() { // 반납
		if (state == STATE_BORROWED) { // 대출중인지 확인 true
			System.out.println("\"" + super.getCdTitle() + "\" 이(가) 반납 되었습니다."); // "이것이 자바다" 가 반납되었습니다.
	...
	public void printState() { // "이것이 자바다" 는(은) 대출 가능 입니다. 출력
		if (state == STATE_NOMAL) {
			System.out.println("\"" + super.getCdTitle() + "\" CD 가 대출 가능합니다.");
		} else if (state == STATE_BORROWED) {
			System.out.println("\"" + super.getCdTitle() + "\" CD 가  대출중입니다.");
		} else {
