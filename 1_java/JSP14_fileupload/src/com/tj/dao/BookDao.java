...
public class BookDao {
		...
		try {
			...
			if (rs.next()) {
				do {
					...
					dtos.add(new BookDto(bid, bname, bprice, bimage1, bimage2, bcontent, bdiscount));
				} while (rs.next());
			} else {
	...
	public ArrayList<BookDto> listBook(int startRow, int endRow) {
		...
		String selectSql = "SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT * FROM BOOK ORDER BY BID DESC) A) " + 
	...
	public BookDto getBook(int bid) {
		...
		try {
			...
			if (rs.next()) {
				...
				dto = new BookDto(bid, bname, bprice, bimage1, bimage2, bcontent, bdiscount);
			} else {
				System.out.println("가져올 데이터가 없습니다.");
			}
